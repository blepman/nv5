 ...
 *
 * Repoet er privat — du må oppgi GitHub token (PAT) med read på repoet.
 *
 * 1. Last opp til nv5 site root (www/nv5 — mappen kan være tom).
 * 2. Åpne https://nv5.haatetepe.no/nv5-init.php
 * 3. Fyll inn admin-bruker, passord og GitHub-token — fila slettes ved suksess.
 */

const NV5_INIT_OWNER = 'blepman';
const NV5_INIT_REPO = 'nv5';
...
function nv5_init_http_get(string $url, string $token = ''): string
{
    $headers = [
    ...
    if (function_exists('curl_init')) {
        ...
        if ($status >= 400) {
            if ($status === 401) {
                throw new RuntimeException('GitHub HTTP 401 — ugyldig token.');
            }
            if ($status === 404) {
                throw new RuntimeException(
                    'GitHub HTTP 404 — fant ikke ' . NV5_INIT_OWNER . '/' . NV5_INIT_REPO
                    . '. Sjekk at repoet er omdøpt til nv5 og at token har read-tilgang.'
                );
            }
...
function nv5_init_verify_token(string $token): void
{
    nv5_init_http_get(
        'https://api.github.com/repos/' . rawurlencode(NV5_INIT_OWNER) . '/' . rawurlencode(NV5_INIT_REPO),
        $token
    );
}
...
function nv5_init_rm_tree(string $dir): void
{
    if (!is_dir($dir)) {
        return;
    }
...
function nv5_init_copy_file(string $from, string $to): void
{
    if (!is_file($from)) {
        throw new RuntimeException('Mangler fil i server-zip: ' . basename($from));
    }
    $data = file_get_contents($from);
    if ($data === false) {
        throw new RuntimeException('Kunne ikke lese ' . $from);
    }
    nv5_init_write_atomic($to, $data);
}

/**
 * @return list<string>
 */
function nv5_init_install_server_skeleton(string $siteRoot, string $token): array
{
    if (!class_exists('ZipArchive')) {
        throw new RuntimeException('PHP ext-zip mangler — nødvendig for oppsett.');
    }

    $url = 'https://api.github.com/repos/'
        . rawurlencode(NV5_INIT_OWNER) . '/' . rawurlencode(NV5_INIT_REPO)
        . '/zipball/' . rawurlencode(NV5_INIT_SERVER_BRANCH);
    $zipData = nv5_init_http_get($url, $token);
    $zipPath = sys_get_temp_dir() . '/nv5-init-' . bin2hex(random_bytes(6)) . '.zip';
    $extract = sys_get_temp_dir() . '/nv5-init-x-' . bin2hex(random_bytes(6));
    ...
    try {
        ...
        if ($zip->open($zipPath) !== true) {
            throw new RuntimeException('Kunne ikke åpne server-zip fra GitHub');
        }
        if (!is_dir($extract) && !mkdir($extract, 0755, true) && !is_dir($extract)) {
            throw new RuntimeException('Kunne ikke lage temp-mappe');
        }
        $zip->extractTo($extract);
        $zip->close();

        $entries = array_values(array_filter(scandir($extract) ?: [], static fn(string $n): bool => $n !== '.' && $n !== '..'));
        if ($entries === []) {
            throw new RuntimeException('Tom server-zip fra GitHub');
        }
        $sourceRoot = $extract . '/' . $entries[0];

        foreach (nv5_init_server_files() as $relative) {
            $from = $sourceRoot . '/' . $relative;
            $dest = $siteRoot . '/' . $relative;
            nv5_init_copy_file($from, $dest);
            $written[] = $relative;
        }
    } finally {
        @unlink($zipPath);
        nv5_init_rm_tree($extract);
    }
...
function nv5_init_page(string $title, string $body, bool $ok = true): void
{
    ...
    echo 'label{display:block;margin:.75rem 0 .25rem}input{width:100%;padding:.55rem .65rem;border-radius:.4rem;border:1px solid #2d4570;background:#121a2b;color:#e8edf7;box-sizing:border-box}';
...
if (!$isPost) {
    nv5_init_page(
        'NV5 oppsett',
        '<p>Sett opp miljøet: henter kode fra privat GitHub-repo <code>' . htmlspecialchars(NV5_INIT_OWNER . '/' . NV5_INIT_REPO, ENT_QUOTES, 'UTF-8') . '</code>, '
...
if (strlen($token) < 10) {
    $errors[] = 'GitHub-token må være minst 10 tegn.';
}
...
try {
    $log = [];
    $log[] = 'Verifiserer GitHub-token mot ' . NV5_INIT_OWNER . '/' . NV5_INIT_REPO . '…';
    nv5_init_verify_token($token);
    $log[] = '  ✓ token OK';

    $log[] = 'Installerer server-skjelett fra GitHub (' . NV5_INIT_SERVER_BRANCH . ')…';
    foreach (nv5_init_install_server_skeleton($siteRoot, $token) as $file) {
        $log[] = '  ✓ ' . $file;
    }
